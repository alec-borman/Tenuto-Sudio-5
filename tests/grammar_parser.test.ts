...
import { Parser, Measure, MacroDef, Assignment } from '../src/parser/Parser';
...
describe('TDB-545: Grammar Parser & Hierarchical AST', () => {
  it('parses a multi-voice measure into nested AST nodes', () => {
    ...
    const measures = score.topLevels.filter(tl => tl instanceof Measure);
    expect(measures).toHaveLength(1);
    
    const measure = measures[0] as Measure;
    expect(measure.number).toBe("1");

    const assignment = measure.logic[0] as Assignment;
  ...
  it('parses a macro definition', () => {
    ...
    const macros = score.topLevels.filter(tl => tl instanceof MacroDef) as MacroDef[];
    expect(macros).toHaveLength(1);
    expect(macros[0].name).toBe('Motif');
  });
});
