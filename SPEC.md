...
5.  **Reconciliation:** The text change triggers the WASM compiler. The compiler generates a fresh Intermediate Representation (IR). The Canvas receives the new IR, drops its optimistic state, and renders the true mathematical geometry. Because the translation was deterministic, the block visually remains exactly where the user dropped it.
   ...
   **Addendum A: Deterministic Phase Alignment & The Atomic Buffer Swap**

To maintain absolute acoustic parity during heavy AST recompilations and bi-directional UI mutations (e.g., massive polyphonic block drags), the `TenutoProcessor` MUST enforce a strict buffer-swapping protocol that rejects variable-latency heuristics in favor of deterministic mathematics. 

This ensures the system never violates the "Never Stop the Music" rule, providing absolute Proof of Inference between the textual representation and the physical audio output.
